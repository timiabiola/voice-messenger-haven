...
-- Create an admin function to fix all unconfirmed phones
CREATE OR REPLACE FUNCTION admin_fix_unconfirmed_phones()
RETURNS TABLE(user_id uuid, email text, phone text, status text) AS $$
BEGIN
  RETURN QUERY
  WITH fixed_users AS (
    SELECT 
  ...
  RETURNING fu.id, fu.email, fu.phone, fu.fix_status;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- Run the admin fix function
SELECT * FROM admin_fix_unconfirmed_phones();
...
COMMENT ON FUNCTION admin_fix_unconfirmed_phones() IS 'Admin function to fix all unconfirmed phone numbers in bulk'; 